...
function App() {
	...
	const [state, setState] = useState({
		AppConstants: {
			menuJsPath: isOldVersion ? "div > div > div > div > div > div#navbar-right-container" : "#__next > div > div > div > nav > div > div > div.relative.ml-4.flex.items-center.space-x-4",
			companyTagsContainerJsPath: isOldVersion ? "div > div > div.main__2_tD > div > div > div > div > div > div > div > div[data-key] > div" :
